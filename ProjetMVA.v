...
From Coq Require Import Arith List Recdef.
...
Proof.
  unfold permutation. intros l1 l2 l3 H12 H23 x.
  specialize (H12 x); specialize (H23 x).
  destruct H12, H23. split.
...
Proof.
  unfold permutation. intros l1 l2 H x.
  specialize (H x). destruct H. split; symmetry; assumption.
Qed.

Lemma permutation_C : forall a l1 l2, permutation l1 l2 -> permutation (a::l1) (a::l2).
Proof.
  unfold permutation. intros a l1 l2 H x.
  specialize (H x). destruct H. simpl. split.
...
Lemma In_app_custom : forall (x : nat) l1 l2, In x (l1 ++ l2) <-> In x l1 \/ In x l2.
Proof.
...
Proof.
  induction l; simpl; intros.
  - auto.
  - destruct (a <=? y) eqn:E; simpl in H.
    + destruct H; [left; auto | right; apply IHl; auto].
...
Proof.
  intros x l. functional induction (QS l); intros.
  - (* Cas nil *) assumption.
  - (* Cas hd :: tl *)
    simpl in H. destruct H as [H_is_hd | H_in_tl].
    + (* x est le pivot (hd) *)
      subst. rewrite In_app_custom. simpl. right. left. reflexivity.
    + (* x est dans le reste de la liste (tl) *)
      rewrite In_app_custom.
      destruct (Nat.leb x hd) eqn:E.
      * (* Cas x <= hd *)
        left. apply IHl0. apply lt_LoE_rev; auto. apply leb_complete; auto.
      * (* Cas x > hd *)
        right. simpl. right. apply IHl1. apply lt_High_rev; auto. apply leb_complete_conv; auto.
Qed.
...
Proof.
  ...
  - (* Cas hd :: tl *)
    rewrite In_app_custom in H. 
    simpl in H. destruct H as [H_in_low | [H_is_hd | H_in_high]].
    + (* x est dans la partie LoE *)
      apply IHl0 in H_in_low. 
      apply In_LoE in H_in_low. 
      right. assumption.
    + (* x est le pivot *)
      subst. left. reflexivity.
    + (* x est dans la partie High *)
      apply IHl1 in H_in_high. 
      apply In_High in H_in_high. 
      right. assumption.
Qed.
...
Proof.
  ...
  - destruct (x =? a) eqn:E.
    ...
    + apply Nat.eqb_neq in E.
      split; intros.
      * apply IHl. intros H1. apply H. simpl. right. assumption.
      * intros [H1 | H2]; [subst; tauto | apply IHl in H; auto].
Qed.
...
Proof.
  ...
  destruct (x =? a); rewrite IHl1; auto.
Qed.

Lemma count_LoE_count : forall x l, count x (LoE x l) = count x l.
Proof.
  induction l; simpl; auto.
  destruct (a <=? x) eqn:E; simpl.
  - destruct (x =? a) eqn:Ex; rewrite IHl; auto.
  - destruct (x =? a) eqn:Ex; auto.
